...
function onInput(e) {
  ...
  if (t.length >= minCharactersToSearch) {
    flattenList = getListWithKeyword([], items, t);
  } else {
    flattenList = [];
  }
}
...
function getListWithKeyword(keywords, tree, t) {
  ...
  for (let i = 0; i < tree.length; i++) {
    const k = keywordsFunction(tree[i]);
  ...
  <div tabindex="0"
    ...
    {#if items.length>0 && flattenList.length === 0 && !searching}
      <div class="flex">
        {#each
        layers as items,i }
          <ul style="max-height: 320px;min-width:180px;"
            ...
            {#each items as item}
              <li
                ...
                {item[labelFieldName]}
                {#if item.children && item.children.length>0}
                  <span class="material-icons ml-3 w-3">keyboard_arrow_right</span>
                {/if}
              </li>
            {/each}
          </ul>
